---
title: "Preprocesamiento y descripción de imágenes con OpenCV"
description: "Evaluación del preprocesamiento de imágenes con OpenCV: espacios de color, contraste, filtrado, detección de esquinas y descriptores locales."
---
...
- Leer una imagen como matriz y comprobar su rango y canales.
- Elegir RGB, HSV o Lab según la pregunta.
- Comparar ecualización global con CLAHE.
- Quitar ruido sin borrar los bordes.
- Detectar esquinas y comparar ORB con SIFT.

## Contexto técnico

Antes de clasificar o detectar objetos hay que saber qué imagen recibió el modelo. El mismo paso sirve para **visión por computadora**, **reconocimiento facial**, **vehículos autónomos** o **análisis médico**, pero los parámetros no son intercambiables.

En esta práctica revisé cómo:
...
Una imagen digital es una **matriz de píxeles**: cada valor representa intensidad o color.
...
Un histograma muestra cómo se reparten las **intensidades** de la imagen.
...
La ecualización global reparte las intensidades por todo el rango disponible.
...
**Ventaja:** mejora el contraste global.
**Limitación:** puede amplificar el ruido porque no considera el contexto local.

#### 6.2 CLAHE (Adaptive Histogram Equalization)

CLAHE mejora el contraste por ventanas (tiles), no con una única transformación para toda la imagen.
...
**Decisión:** en estas imágenes preferí CLAHE porque **preserva detalles locales** sin amplificar tanto el ruido.

### 7. Filtros de Suavizado

Los filtros reducen ruido antes de buscar características.
...
Harris marca puntos donde la intensidad cambia en varias direcciones.
...
Shi-Tomasi parte de Harris y se queda con las N esquinas más prometedoras.
...
ORB es un detector y descriptor rápido, libre de patentes y resistente a la rotación.
...
SIFT suele acertar mejor en matching preciso, a cambio de más tiempo y de `opencv-contrib`.
...
El matching compara los descriptores de dos imágenes.
...
**En estas pruebas CLAHE funcionó mejor porque:**
...
La elección depende de la tarea: HSV ayuda a separar color, Lab a comparar tonos, CLAHE a recuperar detalle local y ORB o SIFT a encontrar puntos repetibles. Después probaría YOLO o una CNN con un conjunto de imágenes separado, y mediría qué preprocesamiento mejora el resultado en vez de asumir que uno sirve para todo.
