---
title: "Cómo leer un archivo de audio"
description: "Recorrido por señales de audio con librosa: forma de onda, FFT, espectrogramas, MFCCs y un pipeline sencillo para clasificación."
---

# Audio como dato: del archivo a las features

## Qué quería sacar de la señal

- Entender qué significan sampling rate, duración, amplitud y canales.
- Mirar la señal en el tiempo y en frecuencia.
- Convertirla en MFCCs y otras features que un modelo pueda usar.
- Comparar una clasificación sencilla con algunas lecturas de tempo y pitch.

## Por qué convertir sonido en números

La misma señal aparece en tareas distintas:
...
Para trabajar con ella hay que:
- **Cargar y normalizar** archivos de audio en diversos formatos
- **Extraer features estables** para entrenar modelos
...
Un audio digital es una **secuencia de muestras** tomadas a intervalos regulares:
...
**Nota:** `librosa.load()` remuestrea por defecto a 22050 Hz y convierte la señal a mono.

### 4. Visualización: Forma de Onda (Waveform)

La forma de onda pone la **amplitud en función del tiempo**.
...
La **FFT (Fast Fourier Transform)** separa la señal en componentes de frecuencia.
...
Un espectrograma muestra cómo cambia el **contenido frecuencial con el tiempo**.
...
**Para qué sirve:** ver formantes de voz, patrones rítmicos o la entrada de un instrumento.

### 7. Espectrograma Mel

La escala **Mel** agrupa las frecuencias de una forma más cercana a la percepción humana y conserva más detalle en las bajas.
...
**Ventaja:** suele ser una entrada práctica para modelos que necesitan una representación parecida a la percepción humana.

### 8. Extracción de Características

#### 8.1 MFCCs (Mel-Frequency Cepstral Coefficients)

Los **MFCCs** resumen la forma espectral de una señal y suelen funcionar bien como entrada para una primera clasificación.
...
**Escalas:** las features de audio viven en rangos distintos (por ejemplo, ZCR ~0.1 y spectral centroid ~2000).

**Solución:** ajustar `StandardScaler` con el conjunto de entrenamiento y reutilizarlo en validación y prueba.

## Qué queda de esta primera pasada

Con librosa pude:

1. **Cargar y manipular** señales de audio en diversos formatos
2. **Visualizar** tanto en dominio temporal como frecuencial
3. **Extraer características estables** (MFCCs, spectral features) para ML
4. **Implementar pipelines completos** de clasificación
5. **Analizar patrones musicales** (tempo, pitch, armonía)

**Aplicaciones que se abren:**
...
**Pipeline típico ML:**
1. Carga de audio → preprocesamiento → extracción de features → modelo → predicción

## Extra: Data Augmentation para Audio

En audio es común empezar con pocos ejemplos. **Data Augmentation** permite generar variaciones controladas para que el modelo no memorice una única voz o interpretación.
...
Con estas variaciones, una palabra rápida, lenta, grave o aguda puede conservar la misma etiqueta.
...
La señal cambia de forma según la representación: waveform para el tiempo, FFT para sus frecuencias, espectrogramas para ver ambas y MFCCs para alimentar un modelo pequeño. Probaría una CNN sobre espectrogramas y compararía su resultado con el pipeline de MFCCs; después mediría separación de fuentes y detección de eventos con datos etiquetados.
