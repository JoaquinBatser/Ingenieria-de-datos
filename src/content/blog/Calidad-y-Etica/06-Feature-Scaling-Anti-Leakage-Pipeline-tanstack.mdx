---
title: "Escalado y pipelines sin data leakage"
description: "Comparación de scalers y transformadores con Ames Housing, incluyendo el split correcto para que el preprocesamiento no contamine la validación."
---
import {
...
**Lectura:** `Lot Area` va de 1,300 a 215,245 y `Overall Qual` de 1 a 10. El ratio supera 165:1. KNN y SVM pueden dejar que `Lot Area` domine la distancia aunque `Overall Qual` sea útil.
...
**Lectura:** Yeo-Johnson puede trabajar con ceros y negativos, a diferencia de Box-Cox. Es una opción para `SalePrice` cuando una transformación log directa no es válida.

<Unit06QuantileTransformer client:load />

**Lectura:** `QuantileTransformer` lleva primero la distribución a una uniforme y luego a una normal. Puede corregir sesgos fuertes, pero cuesta más y puede crear artefactos con muchos valores repetidos.

<Unit06MaxAbsScaler client:load />

**Lectura:** `MaxAbsScaler` divide por el máximo absoluto y deja cada columna en [-1, 1]. Conserva el signo y funciona bien con datos dispersos, pero no centra las columnas en cero.

<Unit06NormalizerL2 client:load />

**Lectura:** `Normalizer` trabaja fila por fila y deja cada muestra con norma L2 igual a 1. Sirve cuando se comparan vectores completos, por ejemplo en texto o imágenes; no sustituye al escalado por columna en una regresión común.

<Unit06FunctionTransformer client:load />

**Lectura:** La transformación log comprime valores grandes y reduce la asimetría hacia la derecha. Solo puede aplicarse cuando los valores son estrictamente positivos, así que `SalePrice` y `Lot Area` requieren ese chequeo.

<Unit06LogComparison client:load />

**Lectura:** Después del log, `SalePrice` queda más simétrica. La combinación puede ayudar antes de `StandardScaler`, pero conviene verificar la forma de cada columna en lugar de aplicarla por rutina.
