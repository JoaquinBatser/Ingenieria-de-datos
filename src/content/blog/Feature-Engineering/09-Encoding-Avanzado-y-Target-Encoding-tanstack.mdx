---
title: "Evaluación de métodos de codificación categórica"
description: "Comparación de Label, One-Hot, Target, Leave-One-Out, Binary y CatBoost Encoding sobre Adult Income, con control de cardinalidad y fuga de información."
---
import {
  EncodingCardinalityChart,
  EncodingExtraResultsChart,
  EncodingFeatureDistributionChart,
  EncodingFeatureImportanceChart,
  EncodingResultsChart,
} from "@/components/tanstack-comparison-09-12/unit-09-charts"

# Codificación de variables categóricas sin ordinalidad artificial

_El código y análisis presentado en este documento refleja el trabajo del autor._

## Objetivos de la comparación

- Ver qué supone cada encoding para una categoría.
- Medir precisión, número de columnas y tiempo.
- Separar el tratamiento de columnas de baja y media cardinalidad.
- Comprobar la importancia con Random Forest y SHAP.
- Ajustar Target Encoding sin que el target se filtre al test.
...
**Objetivo:** comparar los encodings con el mismo modelo y la misma partición.

**Restricción:** One-Hot genera más de 100 columnas y aumenta la dimensión del problema.
...
Separé las columnas por número de categorías antes de elegir un encoding:
...
<EncodingCardinalityChart client:load />

**Interpretación:** `native-country` tiene 42 categorías; One-Hot añadiría 41 columnas para esa variable. La cifra explica por qué conviene comparar una representación más compacta.

### 3. Experimento 1: Label Encoding

Label Encoding asigna un entero a cada categoría. Es fácil de aplicar, pero ese número puede parecer un orden que la variable no tiene.
...
**Conclusión:** Tiene sentido en modelos que no interpreten ese entero como una distancia, como Random Forest o Gradient Boosting.
...
**Conclusión:** Reduce columnas en variables con >30 categorías, pero solo es fiable si se ajusta sin mirar el target de validación.
...
<EncodingResultsChart client:load />

**Interpretación:** Label Encoding consigue el mejor AUC (0.91), mientras que el Pipeline Branched queda más equilibrado entre Accuracy, AUC y F1. La elección depende de qué métrica y qué mantenimiento importen.
...
<EncodingFeatureImportanceChart client:load />

**Interpretación:** Las variables numéricas concentran 76.6% de la importancia; `fnlwgt` y `age` encabezan el ranking. Las categóricas aportan, pero no son la señal principal en este modelo.

<EncodingFeatureDistributionChart client:load />

**Interpretación:** SHAP deja ver interacciones no lineales, en especial alrededor de `age`, que no aparecen en un ranking único de importancia.
...
**Interpretación:**
- **Target Encoding** usa menos columnas en datasets grandes, siempre que el ajuste respete la validación.
- **Pipeline Branched** separa el tratamiento de cada grupo de columnas y deja el flujo más fácil de revisar.
- **Label Encoding** no conviene cuando las categorías no tienen un orden natural.

### 10. Extensión: Aplicación en Ames Housing

Para comprobar si el resultado se repetía, llevé las técnicas a Ames Housing, convertido en un problema binario:
...
**Aprendizaje:** El procedimiento se puede trasladar a otro dominio, pero el encoding ganador debe medirse de nuevo.
...
2. **Target Encoding reduce mucho la dimensionalidad** (94 → 6)
...
Como prueba adicional comparé **CatBoost Encoding** con **Target Encoding** en dos modelos.
...
<EncodingExtraResultsChart client:load />

**Interpretación:** Gradient Boosting obtiene los mejores valores con ambos encodings; la diferencia a favor de CatBoost es ligera. El modelo final pesa tanto como el encoding.
...
4. **Target Encoding más versátil:** funciona con Random Forest y Gradient Boosting, así que deja más margen si el modelo cambia.
