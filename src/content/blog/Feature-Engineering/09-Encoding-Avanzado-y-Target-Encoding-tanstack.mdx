---
title: "Cómo codificar categorías sin perder información"
description: "Comparación de Label, One-Hot, Target, Leave-One-Out, Binary y CatBoost Encoding sobre Adult Income, con atención al leakage y a la cardinalidad."
---
import {
...
# Codificar categorías sin inventar un orden
...
**Lectura:** `native-country` tiene 42 categorías; One-Hot añadiría 41 columnas para esa variable. La cifra explica por qué conviene comparar una representación más compacta.
...
**Lectura:** Label Encoding consigue el mejor AUC (0.91), mientras que el Pipeline Branched queda más equilibrado entre Accuracy, AUC y F1. La elección depende de qué métrica y qué mantenimiento importen.
...
**Lectura:** Las variables numéricas concentran 76.6% de la importancia; `fnlwgt` y `age` encabezan el ranking. Las categóricas aportan, pero no son la señal principal en este modelo.

<EncodingFeatureDistributionChart client:load />

**Lectura:** SHAP deja ver interacciones no lineales, en especial alrededor de `age`, que no aparecen en un ranking único de importancia.
...
**Lectura:** Gradient Boosting obtiene los mejores valores con ambos encodings; la diferencia a favor de CatBoost es ligera. El modelo final pesa tanto como el encoding.
