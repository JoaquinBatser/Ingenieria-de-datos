---
title: "Ingeniería de variables derivadas con pandas"
description: "Construcción y evaluación de razones, transformaciones y variables compuestas con pandas para un modelo de valoración inmobiliaria."
---
import {
...
# Construcción y evaluación de variables derivadas

## Objetivos del análisis

- Crear ratios, transformaciones y scores a partir de columnas existentes.
- Evaluar si las nuevas columnas modifican la distribución o incorporan ruido.
- Comparar Mutual Information con la importancia de Random Forest.
- Repetir el procedimiento sobre datos sintéticos y Ames Housing.
...
### 1. Configuración y generación del conjunto de datos sintético
...
**Interpretación:** `log_price` queda más simétrica que la variable original. La transformación cambia la forma de la distribución; el efecto sobre un modelo hay que medirlo aparte.
...
**Interpretación:** Las nuevas features añaden detalle y la mayoría queda por debajo de 5% de outliers. Aun así, cada valor extremo necesita revisión antes de eliminarlo.

<Unit08FeatureDistributionAnalysis client:load />

**Interpretación:** La mayoría de las columnas nuevas tiene menos de 5% de outliers; `property_age` queda en 0%. El gráfico muestra el recuento, no decide si esos casos son errores.
...
**Interpretación:** `sqrt_sqft` aparece alto en MI. Es una señal de dependencia no lineal, no una prueba de que la transformación sea necesaria en cualquier dataset.
...
**Interpretación:** `crime_rate` y `school_rating` pesan más que varias medidas estructurales en este Random Forest. El árbol puede capturar interacciones que la correlación lineal no ve.
...
**Interpretación:** Con features derivadas, R² pasa de 0.89 a 0.95 y MSE de 7.90 a 3.43. Es una mejora en este experimento; hace falta validarla con otra partición y otro dataset.

<Unit08BostonFeatureImportance client:load />

**Interpretación:** `price_per_room` queda primero tanto en Random Forest como en MI. El acuerdo entre métodos es útil aquí, pero no convierte al ratio en una regla universal.
