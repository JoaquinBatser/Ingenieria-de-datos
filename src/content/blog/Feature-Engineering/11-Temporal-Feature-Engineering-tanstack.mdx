---
title: "Ingeniería de variables temporales con pandas"
description: "Construcción de rezagos, ventanas móviles, métricas RFM y variables cíclicas sobre Online Retail mediante validación temporal estricta."
---
import {
  TemporalCyclicEncodingChart,
  TemporalFourierAnalysisChart,
  TemporalModelComparisonChart,
  TemporalRfmDistributionsChart,
  TemporalRollingCartChart,
  TemporalWindowActivityChart,
} from "@/components/tanstack-comparison-09-12/unit-11-charts"

# Modelado del comportamiento de compra mediante variables temporales


## Objetivos del análisis

- Guardar la historia de cada usuario con `.shift()`.
- Resumir tendencias recientes con rolling y expanding windows.
- Construir RFM y ventanas de 7d, 30d y 90d.
- Validar siempre hacia adelante para no usar el futuro.
- Representar horas y días con sin/cos.
- Añadir indicadores externos usando solo información ya conocida.

## Definición del problema

**Online Retail** contiene transacciones del Reino Unido entre 2010 y 2011. Quise saber si la historia de compras ayuda a anticipar una recompra y a separar usuarios que están perdiendo actividad.
...
<TemporalWindowActivityChart client:load />

**Interpretación:** los días entre órdenes varían mucho y la mediana ronda ~36 días. Esa dispersión justifica guardar lags y ventanas, no solo un promedio global.

### 2. Agregación a Nivel de Orden

Para calcular historia por cliente primero convertí las líneas en una fila por factura.
...
**Por qué `.groupby()` + `.shift()` evita leakage:**
...
Las ventanas móviles resumen los últimos N eventos y dejan fuera la orden actual.
...
<TemporalRollingCartChart client:load />

**Interpretación:** el rolling mean de las últimas 3 órdenes suaviza el cart size actual. La diferencia entre ambos muestra cambios recientes sin mirar la compra que se quiere predecir.
...
Las ventanas expandibles acumulan la historia disponible hasta “ahora”.
...
<TemporalRfmDistributionsChart client:load />

**Interpretación:** `Monetary` tiene una cola larga: algunos usuarios gastan mucho más que otros. `Recency`, `Frequency` y `Monetary` aportan señales distintas.
...
**Interpretación:** RFM separa cuánto hace que compró alguien, con qué frecuencia y cuánto gasta; las tres medidas se complementan.

### 7. Time Window Aggregations (7d, 30d, 90d)

Las ventanas de tiempo comparan actividad reciente y de mediano plazo.
...
**Interpretación:** comparar ventanas ayuda a encontrar usuarios que se están activando (7d > 30d/3) o quedando inactivos (7d = 0 pero 90d > 0).

### 8. Product Diversity Features

Las métricas de diversidad indican si el usuario repite siempre lo mismo o explora productos distintos.
...
<TemporalCyclicEncodingChart client:load />

**Interpretación:** en el espacio sin/cos, las 23h quedan cerca de las 0h y el domingo cerca del lunes. Esa continuidad evita que el modelo trate el reloj como una línea con extremos alejados.
...
Las variables externas añaden contexto macro, siempre que se rellenen solo con datos ya publicados.
...
**Restricción temporal:** aplicar únicamente *forward fill* (`ffill`); el uso de *backward fill* (`bfill`) introduciría información futura.
...
**Interpretación:** el modelo con variables temporales alcanza un AUC 8.7 puntos porcentuales mayor en esta validación.
...
### 14. Otra prueba: churn

Para comprobar si el patrón se repetía, llevé las mismas ideas a **churn prediction** en telecomunicaciones.
...
**Interpretación:** no apareció evidencia clara de data leakage; los folds respetan el orden temporal y las features principales usan historia.
...
Como extensión probé **Fourier** (FFT) y **Seasonal Decomposition** para buscar ciclos que los lags manuales no capturan.
...
<TemporalFourierAnalysisChart client:load />

**Interpretación:** FFT marca las frecuencias semanal (1/7) y mensual (1/30); la descomposición separa tendencia, estacionalidad y residual.

<TemporalModelComparisonChart client:load />

**Interpretación:** sumar Fourier a las features manuales mejora el AUC en +1.14% en esta validación. La ganancia es pequeña, pero la señal es complementaria.
...
5. **Una señal adicional:** el espectro hace visibles ciclos semanales (1/7) y mensuales (1/30) que no se ven en una tabla de lags. Time Series Cross-Validation sigue siendo la prueba que evita engañarse con el futuro.
...
1. **Las temporal features cambian la métrica:** el modelo llega a AUC 0.7204 frente a 0.6625 sin ellas (+8.7% improvement).
