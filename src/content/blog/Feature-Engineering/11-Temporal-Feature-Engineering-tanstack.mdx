---
title: "Features temporales con pandas"
description: "Cómo construir lags, ventanas móviles, RFM y variables cíclicas con Online Retail, validando siempre con el tiempo correcto."
---
import {
...
# Qué compran los usuarios cuando miro el tiempo
...
**Lectura:** los días entre órdenes varían mucho y la mediana ronda ~36 días. Esa dispersión justifica guardar lags y ventanas, no solo un promedio global.
...
**Lectura:** el rolling mean de las últimas 3 órdenes suaviza el cart size actual. La diferencia entre ambos muestra cambios recientes sin mirar la compra que se quiere predecir.
...
**Lectura:** `Monetary` tiene una cola larga: algunos usuarios gastan mucho más que otros. `Recency`, `Frequency` y `Monetary` aportan señales distintas.
...
**Lectura:** RFM separa cuánto hace que compró alguien, con qué frecuencia y cuánto gasta; las tres medidas se complementan.
...
**Lectura:** comparar ventanas ayuda a encontrar usuarios que se están activando (7d > 30d/3) o quedando inactivos (7d = 0 pero 90d > 0).
...
**Lectura:** en el espacio sin/cos, las 23h quedan cerca de las 0h y el domingo cerca del lunes. Esa continuidad evita que el modelo trate el reloj como una línea con extremos alejados.
...
**Lectura:** el modelo con variables temporales alcanza un AUC 8.7 puntos porcentuales mayor en esta validación.
...
**Lectura:** no apareció evidencia clara de data leakage; los folds respetan el orden temporal y las features principales usan historia.
...
**Lectura:** FFT marca las frecuencias semanal (1/7) y mensual (1/30); la descomposición separa tendencia, estacionalidad y residual.

<TemporalModelComparisonChart client:load />

**Lectura:** sumar Fourier a las features manuales mejora el AUC en +1.14% en esta validación. La ganancia es pequeña, pero la señal es complementaria.
