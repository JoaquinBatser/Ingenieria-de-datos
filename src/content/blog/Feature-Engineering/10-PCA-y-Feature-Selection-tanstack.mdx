---
title: "Comparación entre PCA y selección de variables"
description: "Comparación de PCA y métodos de selección de variables sobre Ames Housing según varianza retenida, capacidad explicativa y control de fuga de información."
---
import {
  PcaEmbeddingsChart,
  PcaLoadingsChart,
} from "@/components/tanstack-comparison-09-12/unit-10-charts"

# PCA o selección de variables: dos formas de reducir una tabla


## Objetivos de la comparación

- Aplicar PCA y leer su varianza acumulada.
- Seleccionar columnas con métodos filter, wrapper y embedded.
- Medir qué se gana o se pierde en rendimiento e interpretación.
- Mantener el split separado de cualquier decisión de selección.

## Definición del problema

**Ames Housing** trae ~80 características por casa. La pregunta es si hace falta conservarlas todas para predecir `SalePrice`.
...
Con 80+ columnas es razonable esperar información repetida y ruido. `GarageArea` y `GarageCars`, por ejemplo, describen parte de la misma propiedad.

### 3. PCA - Análisis de Componentes Principales

#### 3.1 Estandarización: el requisito de PCA

PCA es sensible a la escala. Hay que estandarizar antes de ajustar los componentes.
...
PCA usa la varianza de cada columna. Si una variable está en metros cuadrados y otra en número de baños, la primera podría dominar por su magnitud, no por su señal.
...
<PcaLoadingsChart client:load />

**Interpretación:** hacen falta 39 componentes para llegar al 80% de varianza. La tabla puede bajar de 81 a 39 columnas y conservar el 80% de la información, una reducción de 51.9%.
...
### 8. La misma prueba en otro dataset

Para comprobar si el resultado era exclusivo de viviendas, llevé la selección al dataset **Adult Income** (clasificación binaria).
...
**Aprendizaje:** los métodos se comportan de forma parecida en este segundo dataset; MI funciona bien para clasificación y Lasso deja una selección compacta.

**Por qué lo elegí:**
- Diferente tipo de problema (clasificación vs regresión)
- Permite validar que las técnicas funcionan en distintos contextos
- Permite comprobar si MI mantiene el resultado en ambos tipos de problema

### 9. Visualizaciones y Análisis

**Análisis comparativo de métodos:**

MI obtiene el mejor resultado y supera al modelo original en esta comparación. La selección quitó columnas que añadían ruido, no solo dimensiones.

**Comparación de importancias:**

`Overall Qual`, `Gr Liv Area` y `Garage Area` aparecen entre las variables importantes en varios métodos. En este dataset, tamaño y calidad son las señales más estables.
...
Como contraste visual comparé **t-SNE** y **UMAP** con PCA.
...
<PcaEmbeddingsChart client:load />

**Interpretación:** t-SNE y UMAP dibujan grupos más claros que PCA en 2D, porque priorizan estructura local. Para modelar, PCA conserva mejor la varianza global.
...
5. **Una regla práctica:** UMAP puede ayudar con relaciones no lineales; PCA sigue siendo más fácil de explicar. La pérdida de performance con PCA (90.7%) solo es aceptable si el objetivo justifica bajar de 20 a 2 dimensiones.
...
### Lo que elegiría según el objetivo
...
5. **Lasso puede reducir dimensionalidad automáticamente**, forzando a 0 features no importantes. En este caso no mejoró de forma clara sobre filter methods
